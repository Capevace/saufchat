...
import { UserError, createPlaybackState, createSkipTask, createQueueItem, createSpotifyTrack } from './models';
export default function setupSpotifySocket(io, socket, roomId, peerId) {
    /**
     * MODELS
     *
     ...
     */
    const safeAction = async (action, done = (message) => { }, message = 'Internal Server Error') => {
        try {
            // Run our action
     ...
     */
    async function playFirstTrackInQueue(roomId, startedAt) {
        ...
        logger.info(`Spotify skip task scheduled in ${executeIn / 1000}s, SkipTask ${JSON.stringify(skipTask)}`);
     ...
     */
    async function runSkipTask(skipTask) {
        ...
        if (playbackState && playbackState.id !== skipTask.playbackId) {
            throw new Error('We already have a different thing playing, not skipping');
        }
        ...
        try {
            ...
            await stopPlayback();
            logger.error(`Spotify could not play first track in queue after skip task`, e);
            // throw e;
        }
    }
    /**
    ...
    async function addTrackToQueue(track) {
        ...
        if (queueLength >= 100) {
            throw new UserError('No more than 100 songs can be added to a queue');
        }
     ...
     */
    async function removeQueueItemAtIndex(index) {
     ...
     */
    // Queue Routes
    socket.on('spotify:get-queue', async (_data, done) => {
        logger.http(`Spotify get queue. RoomID: ${roomId}, PeerID: ${peerId}`);
        safeAction(async () => {
            const queue = await redis.getQueue(roomId);
            return {
                queue
            };
        }, done, 'Could not fetch queue');
    });
    socket.on('spotify:add-track', async ({ trackUri, length }, done) => {
        logger.http(`Spotify add track. RoomID: ${roomId}, PeerID: ${peerId}, TrackURI: ${trackUri} Length: ${length}`);
        safeAction(async () => {
            if (!trackUri || !length) {
        ...
        }, done, 'Could not add track');
    });
    socket.on('spotify:remove-track', async ({ index }, done) => {
        logger.http(`Spotify remove track. RoomID: ${roomId}, PeerID: ${peerId}, Index: ${index}`);
        safeAction(async () => {
    ...
    socket.on('spotify:get-playback', async (data, done) => {
        logger.http(`Spotify get playback. RoomID: ${roomId}, PeerID: ${peerId}`);
        safeAction(async () => {
            return {
                playback: await redis.getPlaybackState(roomId)
            };
        }, done, 'Could not get playback');
    });
    // Called when there's not current playback, and the user started it.
    socket.on('spotify:start-playback', async ({ startedAt }, done) => {
        logger.http(`Spotify new playback. RoomID: ${roomId}, PeerID: ${peerId}, Data: ${JSON.stringify({ startedAt })}`);
        safeAction(async () => {
            ...
            return {
                playback: await playFirstTrackInQueue(roomId, startedAt || Date.now())
            };
        }, done, 'Could not start playback');
    });
    socket.on('spotify:stop-playback', async (data, done) => {
        logger.http(`Spotify stop playback. RoomID: ${roomId}, PeerID: ${peerId}`);
        safeAction(async () => {
            await stopPlayback();
        }, done, 'Could not start playback');
    });

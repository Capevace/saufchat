	...
	io.on('connection', async (socket: SocketIO.Socket) => {
		try {
			...
			if (
				...
				logger.http(
					`Client connected with invalid IDs. RoomID: ${roomId}, PeerID: ${peerId}`
				);
			...
			logger.http(
				`Client joined room. RoomID: ${roomId}, PeerID: ${peerId}`
			);
			...
			socket.on('disconnect', async reason => {
				logger.http(
					`Client disconnected. RoomID: ${roomId}, PeerID: ${peerId}`
				);

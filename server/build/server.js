...
export default function startServer(callback) {
    ...
    app.get('/spotify/webhook', async (req, res) => {
        const loginId = String(req.query['state']);
        ...
        try {
            ...
            const originRoomId = await redis.getOriginRoomForLoginId(loginId);
            res.redirect(`${config.APP_URL}/#/room/${originRoomId}?${redirectParams}`);
        }
        catch (e) {
            res.status(500).json({
    ...
    app.get('*', (req, res) => {
        ...
        res.status(404).send(`<body style="background:black;color:#12db12;font-family:monospace;">${'Successfully deployed api.sauf.chat' || quotes[randomIndex]}</body>`);
    });
